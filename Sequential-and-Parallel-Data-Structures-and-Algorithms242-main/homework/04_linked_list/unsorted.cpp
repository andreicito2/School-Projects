...
void UnsortedType::PutItem(ItemType item) {
  ...
  location->next =
      listData;        // Store address of first node in next field of new node
...
ItemType UnsortedType::MinItem() {
  NodeType *min = listData;
  MinLoc(listData, min);
  return min->info;
}

void UnsortedType::SortItems() { Sort(listData); }
...
std::ostream &operator<<(std::ostream &os, const UnsortedType &list) {
  ...
  if (list.length == 0) {
    ...
    currentPos = list.listData;
    for (int counter = 1; counter <= list.length; counter++) {
      (currentPos->info).Print(os);
      currentPos = currentPos->next;
    }
  }
  os << std::endl;
